...
import lanit.junit.form.fields.validation.MandatoryFieldValidation;
import lanit.junit.selenium.SeleniumPrerequisites;
import org.apache.log4j.Logger;
import org.openqa.selenium.*;

public class ApplyRequestAction extends FieldAction {

    private static Logger log = Logger.getLogger(ApplyRequestAction.class);

    /**
     * Basic step information about apply button
     ...
     */
    public ApplyRequestAction(String fieldType, String fieldIdentification, String fieldIdentificationValue, String fieldValue, String fieldAutoValue, String fieldReadOnly, String fieldFormatFile, String fieldValidation) {
    ...
    public void run(WebDriver driver) {
        ...
        try {
            if (this.fieldIdentification.equals("") &&
                ...
                new MandatoryFieldValidation().preCheck(driver);
                boolean existButton = driver.findElements(By.xpath("//span[@id='submitBtn' and contains(., 'Подать заявление')]")).size() > 0;
                String actualValue = "Кнопка 'Подать заявление' существует: " + existButton;
                driver.findElement(By.id("submitBtn")).click();
                new MandatoryFieldValidation().postCheck(driver);
                assertEquals("Кнопка 'Подать заявление' существует: true", actualValue);
            } else {
